...
type Required = [ SkillConfig, number ] | SkillConfig
...
const unprocessedTree:SkillConfig[] = []

const add = 
  (a:SkillConfig):SkillConfig =>
  ( unprocessedTree.push(a)
  , a
  )

const html:SkillConfig = add(
  { title: 'HTML'
  , requires:[]
  ...
  , levels: 
    [ { description: 'You understand how to create and view a basic web page.'
      , requires: []
      }
  ...
  , links:
    [ [ 'HTML.net Tutorials', 'http://www.html.net/tutorials/html/' ]
    , [ 'Sublime Text 3, a great code editor', 'http://www.sublimetext.com/' ]
    ]
  })

const css:SkillConfig = add(
  { title: 'CSS'
  , requires: 
    [ html 
    ]
  ...
  , levels: 
    [ { description: 'You are familiar with the basics of CSS formatting and the box model.'
      , requires: []
      }
...
const csstools:SkillConfig = add(
  { title: 'CSS Tools'
  , requires: [ css ]
  , links:
  ...
  , levels:
    [ { description: 'You are familiar with variables and nesting.'
      , requires: []
      }
...
const webdesign:SkillConfig = add(
...
const jquery:SkillConfig = add(
  ...
  , levels:
    [ { description: 'You are familiar with manipulating the document object model (DOM) - modifying values, content, properties, entire elements, etc.'
      , requires: []
      }
...
const seo:SkillConfig = add(
  ...
  , levels: []
  , links:
    [ [ 'Search Engine Optimization (Google)', 'https://support.google.com/webmasters/answer/35291' ]
...
const analytics:SkillConfig = add(
  { title: 'Analytics'
  , requires: 
    [ [ seo, 2 ] 
    ]
  , levels: []
  , links:
    [ [ '10 SEO Analysis Tools You Should Be Using', 'http://www.webgnomes.org/blog/10-seo-analysis-tools/' ]
...
const js:SkillConfig = add(
  ...
  , levels: []
  , links:
    [ [ 'JavaScript Tutorial', 'http://www.htmldog.com/guides/javascript/' ]
...
const jslib:SkillConfig = add(
  ...
  , links:[]
  , levels:
    [ { description: 'You tap into libraries like jQuery, MooTools, Prototype, Dojo, and YUI to streamline your work.'
      , requires: []
      }
...
const front_end:SkillConfig = add(
...
const server_side_programming:SkillConfig = add(
  { title: 'Server-side Programming'
  , requires: [ [ html, 2 ], [ js, 2 ] ]
  , levels: []
  , links:
    [ [ 'Server-side scripting Wiki', 'http://en.wikipedia.org/wiki/Server-side_scripting' ]
    ]
...
const server_side_frameworks:SkillConfig = add(
...
const db:SkillConfig = add(
  ...
  , levels:
    [ { description: 'You understand how to setup tables.'
      , requires: []
      }
...
const dbmanagement:SkillConfig = add(
  { title: 'Advanced DB Management'
  , requires: 
    [ db 
    ]
  , links:
    [ [ 'Stored Procedure - Wiki', 'http://en.wikipedia.org/wiki/Stored_procedure' ]
  ...
  , levels:
    [ { description: 'You write stored procedures and user-defined functions for more efficient querying.'
      , requires: []
      }
...
const server_side_development_mastery:SkillConfig = add(
...
const auth:SkillConfig = add(
  { title: 'User Authentication and Authorization'
  , requires: [ server_side_development_mastery ]
  , levels: []
  , links:
    [ [ 'ASP.NET authentication and authorization - CodeProject', 'http://www.codeproject.com/Articles/98950/ASP-NET-authentication-and-authorization' ]
...
const ajax:SkillConfig = add(
  { title: 'AJAX & APIs'
  , requires: [ front_end, server_side_development_mastery ]
  , levels: []
  , links:
    [ [ 'AJAX (programming) Wiki', 'http://en.wikipedia.org/wiki/Ajax_(programming)' ]
...
const user_discovery:SkillConfig = add(
  { title: 'User Discovery'
  , requires:[]
  , levels:
    [ { description: 'You know the right questions to ask, and use sketches to confirm your ideas.'
      , requires: []
      }
...
const graphic_design:SkillConfig = add(
  { title: 'Graphic Design'
  , requires: [ user_discovery ]
  , levels:
    [ { description: 'You can create a balanced, complementary layout with a clear message.'
      , requires: []
      }
  ...
  , links:
    [ [ 'Graphic Design Wiki', 'http://en.wikipedia.org/wiki/Graphic_design' ]
...
const graphic_design_tools:SkillConfig = add(
  { title: 'Graphic Design Tools'
  , requires: [ graphic_design ]
  , levels: []
  , links:
    [ [ '100 Top Tools for Graphic Designers | Graphic Design Classes', 'http://graphicdesignclasses.net/design-tools/' ]
    ]
...
const prototypes:SkillConfig = add(
  { title: 'Prototypes'
  , requires: [ user_discovery ]
  , levels: []
  , links:
    [ [ 'Design Better And Faster With Rapid Prototyping', 'http://www.smashingmagazine.com/2010/06/16/design-better-faster-with-rapid-prototyping/' ]
...
const user_experience_design_mastery:SkillConfig = add(
...
const user_testing:SkillConfig = add(
  { title: 'User Testing'
  , requires: [ user_experience_design_mastery ]
  , levels: []
  , links:
    [ [ 'My big list of 24 Web Site Usability Testing Tools', 'http://www.usefulusability.com/24-usability-testing-tools/' ]
...
const server_admin:SkillConfig = add(
  { title: 'Server Administration'
  , requires:[]
  , links:
    [ [ 'Comparison of web server software Wiki', 'http://en.wikipedia.org/wiki/Comparison_of_web_server_software' ]
    , [ 'Apache mod_rewrite - Apache HTTP Server', 'http://httpd.apache.org/docs/2.4/rewrite/' ]
    ]
  , levels:
    [ { description: 'You are capable of setting up your application in a new environment.'
      , requires: []
      }
...
const deployment:SkillConfig = add(
  { title: 'Deployment'
  , requires: 
    [ server_admin 
    ]
  , links:
    [ [ 'SSL Certificate Installation', 'http://www.sslshopper.com/ssl-certificate-installation.html' ]
  ...
  , levels:
    [ { description: 'You minify your public-facing files and serve compressed files to save bandwidth and improve your site&raquo;s speed.'
      , requires: []
      }
...
const web_design_mastery:SkillConfig = add(
  ...
  , levels: []
  , links:
    [ [ 'Web Development Wiki', 'http://en.wikipedia.org/wiki/Web_development' ]
    ]
  , description: 'This refers to designing, creating, and maintaining a website.'
  })

const makeUidToId = 
  (byID:{[k:string]:ProcessingSkill & {id:number} }) =>
    ( uid:string ) => 
    ( uid in byID
    ? byID[uid].id
    : ( console.log(uid)
...
const buildTree = 
  ( skills:SkillConfig[], byID:{[k:string]:ProcessingSkill & {id:number} }={}, UidToId=makeUidToId(byID)) =>
  ( skills
    .reduce(
      ( base:ProcessingSkill[], { title, requires, links, levels, description }:SkillConfig ) =>
      { const _uid = toSlug(title)
      ; const uid = `${_uid}1`
      ; const final:ProcessingSkill = 
        { title:`${title} 1`
      ...
      ; const children = levels.map(
          ( { description, requires, links }, nid:number ):ProcessingSkill =>
          ( { title:`${title} ${nid+2}`
    ...
    .map(
      ( { title, requires, links, levels, description, level, uid }:ProcessingSkill, id:number ) =>
      ( byID[uid] = 
        { title
        , requires
        , links
    ...
    .map(
      ( { title, requires, links, levels, level, description, uid, id } ):Skill =>
      ( { title
...
const cx =
  ...
  : ( typeof classes == 'string'
    ...
    : ( Array.isArray(classes)
      ? classes.map(cx)
      : Object.keys(classes).map((k)=>(!!classes[k] ? k : false))
      ).filter(Boolean).join(' ')
    ) || undefined
  )
...
type HTMLChild = string|number|Node

type HTMLClassAttr = string|string[]|{[key:string]:any}
...
const isNode =
  ( thing:any ):thing is Node =>
  ( thing && 'nodeName' in thing && 'nodeType' in thing
  )

const isRawValue =
  (thing:any):thing is HTMLChild =>
  ( typeof thing == 'string' || typeof thing == 'number'
  )

const isHTMLChild =
  (thing:any):thing is HTMLChild =>
  ( isRawValue(thing) || isNode(thing)
  )

function createElement( tag:string, props:HTMLProps)
function createElement( tag:string, props:HTMLProps, ...children:HTMLChild[] )
function createElement( tag:string, ...children:HTMLChild[] )
function createElement( tag:string, props:HTMLChild|HTMLProps, ...children:HTMLChild[] )
  { const attrs =
    ( isHTMLChild(props) 
...
const mount = 
  ( root:Node, where:string='body' ) => 
  ( (document.querySelector(where)||document.body).appendChild(root)
  )
...
const anchor = 
  (href:string,text:string=href,props?:any) =>
  ( a({href,...props},text)
  )

const style = 
  ( props:{[key:string]:{[key:string]:any}} ) =>
  ( h('style', Object.keys(props).map(
        ( selector ) =>
        (`${selector}{${cs(props[selector])}}`)
      ).join('')
    )
  )
      ...
      ( div
        ...
        , levels.length && ul
          ...
          , ...levels.map
            ( ( n ) =>
              ( li
                ( anchor(`#${tree[n].uid}`,tree[n].title)
                )
              )
            )
          )
